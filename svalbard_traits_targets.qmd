...
To test the effect of warming on plant height, a linear regression with a normal error distribution was used.
Plant height was the reponse variable and warming treatment the predictor.
...
# run a linear model and use the tidy function to get the model output
mod <- lm(Value ~ Treatment, data = bistorta)
mod_summary <- tidy(mod)

```

In total, we measured the height of 77 *Bistorta vivipera* plants across the different habitats (@tbl-height-means). The *Bistorta vivipera* in the OTC plots is 1.2 cm ± 0.39 taller than in the control plots.
...
# make pretty table with average plant height per habitat
bistorta |> 
